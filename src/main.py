...
from mcq_generator import attach_mcqs_to_topic_json  # ✅ NEW (you already added)
...
# ✅ NEW: how many MCQs per subtopic to attach in JSON (response)
MCQ_PER_SUBTOPIC = 10
# ======================


# ==========================================================
# ✅ NEW: CLEAN \n FROM CONTENT + MCQS (ONLY ADDITION)
# ==========================================================
def normalize_text_for_json(s: str) -> str:
    """
    Convert multi-line / weird whitespace into single-line clean text.
    - replaces \n, \r, \t with space
...
def clean_topic_json(obj: list) -> list:
    """
    Walk through topic->subtopics and:
    - clean subtopic content
    - clean mcq question + source_sentence + options + answer
    Supports mcqs key as: 'mcqs' OR 'questions'
    """
...
def extract_between_tokens(text: str, start_token: str, next_token: str | None) -> str:
    """
    ✅ Real fix:
    Return only the portion that starts at start_token and ends before next_token (if exists).
    Works even when both tokens are on same page.
    """
    if not text:
        return ""
...
def run():
    ...
    # ==========================================================
    # ✅ NEW: Attach MCQs inside each subtopic BEFORE saving JSON
    # (does not change your old flow, only adds mcqs[] in JSON)
    # ==========================================================
    ...
    # ==========================================================
    # ✅ NEW: REMOVE \n FROM content + MCQs BEFORE SAVING JSON
    # ==========================================================
    ...
    # MCQ generation (files) - SAME as before ✅
